...
def test_ingest_models_success():
    ...
    with respx.mock:
        ...
        with patch.dict("os.environ", {"GOOGLE_API_KEY": "sk-test-key"}):
            ...
            assert gemini_call.call_id == "google:gemini-1-5-pro.chat"
            assert gemini_call.provider == "google"
            ...
            assert gemini_call.adapter == "google"
            assert gemini_call.payload_style == "generateContent_v1"
            assert "google:multimodal-cheap" in gemini_call.aliases
