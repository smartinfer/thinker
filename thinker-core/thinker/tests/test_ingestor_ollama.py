...
def test_ingest_success():
    ...
    mock_response = {
        "models": [
            {"name": "llama3:latest", "size": 1000000000, "digest": "sha256:abc123"},
            {"name": "phi3:medium", "size": 500000000, "digest": "sha256:def456"},
        ]
    }
    ...
    with respx.mock:
        ...
        # Check llama3
        llama_call = next(c for c in catalog.calls if c.model_id == "llama3:latest")
        assert llama_call.call_id == "ollama:llama3-latest.chat"
        ...
        # Check phi3
        phi_call = next(c for c in catalog.calls if c.model_id == "phi3:medium")
        assert phi_call.call_id == "ollama:phi3-medium.chat"
        assert phi_call.provider == "ollama"
        assert phi_call.model_id == "phi3:medium"


def test_ingest_custom_base_url():
    """Test Ollama ingestion with custom base URL."""
    mock_response = {"models": [{"name": "llama3:latest", "size": 1000000000}]}
...
def test_ingest_model_name_extraction():
    ...
    with respx.mock:
        ...
        assert "phi3:medium" in model_ids
        assert "codellama:13b" in model_ids
